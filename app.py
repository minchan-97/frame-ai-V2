...
with tab1:
    fw = get_fw()
    st.markdown('<div class="lb l2">오프라인 QA — 코퍼스 검색 기반</div>',
                unsafe_allow_html=True)
    st.caption("코퍼스에서 직접 답변 검색 — 환각 없음 / LLM 불필요")

    if not fw.is_initialized:
        st.info("← 사이드바에서 코퍼스를 학습하세요.")
    else:
        # 대화 이력
        if "qa_offline" not in st.session_state:
            st.session_state.qa_offline = []

        # 대화 표시
        for item in st.session_state.qa_offline[-10:]:
            st.markdown(
                f'<div style="text-align:right;margin:4px 0;">'
                f'<span style="background:#1e3a5f;color:#e2e8f0;'
                f'padding:6px 12px;border-radius:12px 12px 2px 12px;'
                f'display:inline-block;max-width:85%;font-size:0.9rem;">'
                f'{item["q"]}</span></div>',
                unsafe_allow_html=True)

            st_cls = {"PASS":"#00ff88","WARNING":"#ffd600",
                      "FATAL":"#ff4444","SKIP":"#64748b"}.get(
                      item["status"],"#64748b")
            icon   = {"PASS":"🟢","WARNING":"🟡","FATAL":"🔴","SKIP":"⬜"}.get(
                      item["status"],"⬜")
            st.markdown(
                f'<div style="margin:4px 0 12px 0;">'
                f'<span style="background:#111827;color:#e2e8f0;'
                f'border:1px solid {st_cls};'
                f'padding:6px 12px;border-radius:12px 12px 12px 2px;'
                f'display:inline-block;max-width:85%;font-size:0.9rem;">'
                f'{item["a"]}</span>'
                f'<span style="font-size:0.65rem;color:#64748b;margin-left:4px;">'
                f'{icon} {item["status"]} | {item["ms"]:.0f}ms | '
                f'유사도:{item["score"]:.2f}</span>'
                f'</div>',
                unsafe_allow_html=True)

        # 입력
        q_offline = st.text_area(
            "질문", height=68, placeholder="코퍼스 기반으로 답해드릴게요...",
            label_visibility="collapsed", key="q_offline")
        ca, cb = st.columns([4,1])
        with ca:
            send_off = st.button("📤 전송", use_container_width=True,
                                 key="send_offline",
                                 disabled=not q_offline.strip())
        with cb:
            if st.button("🗑️", key="clear_offline"):
                st.session_state.qa_offline = []
                st.rerun()

        if send_off and q_offline.strip():
            # 검색엔진 없으면 즉시 빌드
            if fw.searcher is None or not fw.searcher.sentences:
                fw.searcher = CorpusSearcher() if 'CorpusSearcher' in dir() \
                              else None
                if fw.searcher:
